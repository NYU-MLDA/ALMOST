...
def loadPreTrainedModel(modelDumpPath,test_graphs,device):
    global PRETRAINED_MODEL
    if not (osp.join(modelDumpPath,'model_hyper.pkl') and osp.join(modelDumpPath,'model_model.pth')):
        print("Error: Hyperparams and model files not found. Exiting..")
        exit(0)
    with open(osp.join(modelDumpPath,'model_hyper.pkl'),'rb') as hyperparameters_name:
        saved_args = pickle.load(hyperparameters_name)
    argsDict = {}
    for key, value in vars(saved_args).items(): # replace with saved cmd_args
        argsDict[key] = value
    classifier = GraphCNN(argsDict['num_layers'], argsDict['num_mlp_layers'], test_graphs[0].node_features.shape[1],argsDict['hidden_dim'], num_classes, argsDict['final_dropout'], \
        argsDict['learn_eps'], argsDict['graph_pooling_type'], argsDict['neighbor_pooling_type'], device).to(device)
    if torch.cuda.is_available():
        classifier = classifier.cuda()
    model_name = osp.join(modelDumpPath,'model_model.pth')
    classifier.load_state_dict(torch.load(model_name))
    PRETRAINED_MODEL = classifier
        
class GCNEncoder:
    def __init__(self,parentFolder,links_name,modelDumpPath,device):
        self.parentFolder = parentFolder
        self.links_name = links_name
        self.preTrainedModelPath = modelDumpPath
        self.device = device
    ...
    def processAndGenEmbeddings(self):
        ...
        _, test_graphs,_, max_n_label = links2subgraphs(
                self.A,
                self.B,
                None,
                None,
    ...
    def getOmlaAttackAccuracy(self):
        ...
        if PRETRAINED_MODEL == None:
            print("\nLogging: Loading pretrained model for first time")
            loadPreTrainedModel(self.preTrainedModelPath,test_graphs,self.device)
        acc_test,_,_ = test_new(PRETRAINED_MODEL,self.device,test_graphs)
...
if __name__ == '__main__':
    ...
    print(args.data)
    #exit(0)
    gcnEnc = GCNEncoder(args.data,args.links,args.model,device)
    gcnEnc.prepareData()
