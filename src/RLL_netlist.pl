...
my ($rel_date) = '$Date: 2022/06/29 20:38:38 $' =~ /\: ([\d\/]+) /;
...
format INFO_MSG =
     ...
     @|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
     'New York Univerity, Abu Dhabi, UAE'
...
foreach my $input_file ($input_file) {
    ...
    while (<INPUT_FH>) {
        ...
        elsif ( $line =~ /^\s*(output)\s+.*/ ) {    #############check outputs
...
#	chomp($line);
	...
	/x
      ...
      @ports = split/,/, $3;
@ports=();
                 push @list_of_gates_original, $instance_name;
$instance_name="";
...
# Get just N of them.
my @pick_indexes_false = @shuffled_indexes_false[ 0 .. $key_size - 1 ];  
...
# Get just N of them.
my @pick_indexes = @shuffled_indexes[ 0 .. $key_size - 1 ];  
...
#print "@ports\n";                
	...
	my $edited_line="";
$edited_line=$line;
my $wire_replace="RLL_wire_A_${RLL_track_key}";
...
elsif (  defined($$hash_ref{"D"}) && defined($$hash_ref{"QN"})  ){
...
if ($edited_line=~/\(\s*\Q$output_name\E\s*\)/){          
$edited_line=~s/\(\s*\Q$output_name\E\s*\)/\(\Q$wire_replace\E\)/g;
}
else {print "NEW ERROR\n";

print "Did not find $output_name in $edited_line\n";

print "This is output name! $output_name and this is line $line";
